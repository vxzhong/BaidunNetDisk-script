...
(function () {
    ...
    function queryDir(dir_info, max, num) {
        if (max && num > max) {
            console.log("已经超过: " + max)
            return
        }
        ...
        for (var i = 0; i < obj_length; i++) {
            ...
            if (b_has_dir == 1) queryDir(fileinfo, max, 1 + num); //有目录 递归调用
        }
    }
    ...
    //添加导出按钮
    function addButton() {
        var $dropdownbutton = $('<a class="list-filter" href="javascript:void( 0 );" node-type="btn_export" title="在文件库根目录，选择要导出的文件夹，点击按钮，清单生成后会提示保存文件。\n如果文件夹内部子文件夹很多，需要等待较长时间。8000个子文件夹大概1小时" style="display: inline;">导出目录</a>');
        $('.' + classMap['title']).append($dropdownbutton);
        $dropdownbutton.click(exportDir);

        var $dropdownbutton3 = $('<a class="list-filter" href="javascript:void( 0 );" node-type="btn_export" title="导出任意层级的子文件夹信息。\n需要先在主目录下，选择文件夹所在根目录，点击<ID按钮>获取主目录信息。\n然后可以进入子目录，选择要导出的子目录进行导出!" style="display: inline;">子目录</a>');
        $('.' + classMap['title']).append($dropdownbutton3);
        $dropdownbutton3.click(exportSubDir);

        var $dropdownbutton4 = $('<a class="list-filter" href="javascript:void( 0 );" node-type="btn_export" title="导出任意层级的子文件夹信息。\n需要先在主目录下，选择文件夹所在根目录，点击<ID按钮>获取主目录信息。\n然后可以进入子目录，选择要导出的子目录进行导出!" style="display: inline;">几层子目录</a>');
        $('.' + classMap['title']).append($dropdownbutton4);
        $dropdownbutton4.click(() => {
            var max = prompt("输出几层", "");
            exportSubDir(max);
        });

        var $dropdownbutton2 = $('<a class="list-filter" href="javascript:void( 0 );" node-type="btn_export" title="为了导出子文件夹，需要先获取主目录信息。\n请在文件库根目录下，选择要导出的子目录所在的根目录后执行!" style="display: inline;">ID</a>');
